 ...
 *
 * Ported from cookiechain/cookie-mcp (MIT License) src/core/launchpad/positions.ts @ v0.5.0.
 *
 * The API exposes positions one pool at a time (`/pools/:pool/position/:owner`), which would be
 * one HTTP request per pool. `UserPosition` is a PDA of `["user", pool, owner]` with a fixed
 * layout, so we derive the addresses and batch-read them with `getMultipleAccounts` (100 per
 * call) instead — two round trips cover 100 pools, and the answer is on-chain truth rather than
 * an indexer's view. Same trick for the per-pool `creator_fee_vault` (a plain SPL token account).
 *
 * The first of those two round trips reads the POOL ACCOUNTS themselves, purely for their
 * `owner`: PDA seeds are program-scoped, and the launchpad has been redeployed under a new
 * program id with the old pools left behind on the old one. Deriving every PDA under the pool's
 * own owner is the only version-proof way to do this.
 */
import { PublicKey, type Connection } from "@solana/web3.js";

import type { LaunchpadPosition } from "./types";
import { C } from "./constants";

export const LAUNCHPAD_PROGRAM_ID = new PublicKey(C.LAUNCHPAD_PROGRAM_FALLBACK);

/** Anchor account discriminator (first 8 bytes) from the launchpad IDL. */
export const USER_POSITION_DISCRIMINATOR = Buffer.from(C.USER_POSITION_DISCRIMINATOR);
...
/** `[\"user\", pool, owner]` under `programId` — the deployment that owns the pool. */
...
/**
 * Which program owns a pool is IMMUTABLE — an account's owner never changes once allocated —
 * so this is cached for the life of the process.
 */
...
/**
 * The program that owns each pool account, keyed by pool address. Only pools not already known
 * are read. Pools we can't read are absent AND NOT CACHED (so a transient RPC failure is
 * retried); the caller falls back to the configured id for those.
 */
...
/**
 * A wallet's curve position in each of `pools`, keyed by pool address. Pools the wallet never
 * bought into are simply absent (their PDA does not exist).
 */
